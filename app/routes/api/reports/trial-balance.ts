import type { Route } from "./+types/trial-balance";
import { verifyAuth, authErrorResponse } from "~/lib/auth-middleware";
import { getTrialBalance, seedDefaultChartOfAccounts } from "~/lib/ledger-service";
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    ...
    const report = await getTrialBalance(auth.userId, from, to);
    return Response.json({ success: true, data: report }, { status: 200 });
  } catch (error) {
    console.error("Trial balance GET: failed", error);
    return Response.json(
